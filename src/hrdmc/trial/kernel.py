...
from hrdmc.trial.numba_backend import NUMBA_AVAILABLE, njit
...
if NUMBA_AVAILABLE:
    ...
    @njit(fastmath=False, inline="always")
    def _valid_row_numba(row: FloatArray, rod_length: float) -> bool:
        previous = 0.0
        for particle in range(row.size):
            value = row[particle]
    ...
    def _valid_batch_numba(x: FloatArray, rod_length: float) -> NDArray[np.bool_]:
        ...
        for walker in range(walkers):
            valid[walker] = _valid_row_numba(x[walker], rod_length)
        return valid
    ...
    def _reduced_tg_log_batch_numba(
        ...
        for walker in range(walkers):
            valid = _valid_row_numba(x[walker], rod_length)
            pair_log = 0.0
            reduced_sum = 0.0
