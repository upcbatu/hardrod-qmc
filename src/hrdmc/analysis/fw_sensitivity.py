...
class ForwardWalkingSensitivityResult:
    ...
    shell_comparison: ShellComparison
...
def analyze_fw_observable_sensitivity(
    *,
    ...
    Seed rows are paired between the anchor and candidate.  The density norm
    uses exact histogram widths, while the coarse-grained envelope is compared
    on fixed unit-mass cells inferred once from the aggregate anchor density.
    """
    ...
    return ForwardWalkingSensitivityResult(
        ...
        envelope_relative_l2_by_seed=np.asarray(
            envelope_relative_l2_by_seed,
            dtype=np.float64,
        ),
