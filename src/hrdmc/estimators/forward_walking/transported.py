...
from hrdmc.estimators.forward_walking.assembly import (
    assemble_observable_result_from_stats,
)
from hrdmc.estimators.forward_walking.config import PureWalkingConfig
from hrdmc.estimators.forward_walking.contributions import (
    event_contribution_matrix,
    weighted_density_profile,
)
from hrdmc.estimators.forward_walking.lag_state import (
    LagState,
    SlidingDensityLagState,
    SlidingLagState,
)
from hrdmc.estimators.forward_walking.protocols import FloatArray, TransportEventLike
...
from hrdmc.estimators.forward_walking.status import (
...
class TransportedAuxiliaryForwardWalking:
    ...
    def record_transport_event(self, event: TransportEventLike) -> None:
        ...
        if self._n_walkers is None:
            self._initialize_stream(n_walkers)
        ...
        for observable in self.config.observables:
            if observable == "density":
                self._record_density(event, parent_indices, parent_is_identity, normalized)
                continue
            self._record_scalar(event, observable, parent_indices, parent_is_identity, normalized)
        self._production_event_count += 1

    def _initialize_stream(self, n_walkers: int) -> None:
        self._n_walkers = n_walkers
        self._observable_dimensions = {
            name: self._observable_dimension(name) for name in self.config.observables
        }
        self._observable_configs = {
            name: self.config.for_observable(name) for name in self.config.observables
        }
        self._lag_states = {
            name: {
                lag: self._new_lag_state(
                    ...
                    lag=lag,
                    n_walkers=n_walkers,
                    dimension=self._observable_dimensions[name],
                    config=self._observable_configs[name],
                )
                for lag in self._observable_configs[name].lag_steps
            }
            for name in self.config.observables
        }
        self._direct_reference_sum = {
            name: np.zeros(self._observable_dimensions[name], dtype=float)
            for name in self.config.observables
        }
        self._direct_reference_count = dict.fromkeys(self.config.observables, 0)

    def _record_density(
        self,
        event: TransportEventLike,
        parent_indices: np.ndarray,
        parent_is_identity: bool,
        normalized: FloatArray,
    ) -> None:
        states = self._lag_states["density"]
        density_config = self._observable_configs["density"]
        if self._production_event_count % density_config.collection_stride_steps == 0:
            ...
            self._direct_reference_sum["density"] += density
            self._direct_reference_count["density"] += 1
            states[0].step_density(
    ...
    def _record_scalar(
        ...
        self._direct_reference_sum[observable] += np.sum(normalized[:, np.newaxis] * values, axis=0)
