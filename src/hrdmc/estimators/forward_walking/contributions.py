...
class _ContributionEventLike(Protocol):
    @property
    def positions(self) -> FloatArray: ...
def _raw_r2_contribution_per_walker(
...
def _event_r2_contributions(
    event: _ContributionEventLike,
    *,
    source: str,
    center: float,
    ...
    if source == "raw_r2":
        return _raw_r2_contribution_per_walker(event.positions, center=center)
    if source == "r2_rb":
...
def event_contribution_matrix(
    ...
    *,
    ...
    if observable == "r2":
        values = _event_r2_contributions(
            event,
    ...
    if observable == "density":
        return _density_profile_matrix(event.positions, bin_edges=density_bin_edges)
    raise ValueError(f"unsupported observable: {observable}")
def _density_profile_matrix(positions: FloatArray, *, bin_edges: FloatArray | None) -> FloatArray:
