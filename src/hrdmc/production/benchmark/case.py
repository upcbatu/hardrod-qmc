...
from hrdmc.artifacts.manifest import ensure_dir, write_json_atomic
...
from hrdmc.production.benchmark.fw_aggregation import (
    aggregate_fw_observable_summary,
)
from hrdmc.production.benchmark.selection import (
    benchmark_validation_status,
    pure_fw_validation_status,
    scalar_seed_mean,
)
from hrdmc.production.pure_walking import (
    PureWalkingSeedRun,
    pure_config_for_case,
    pure_config_metadata,
    run_pure_walking_seed_run,
)
from hrdmc.production.stationarity.summary import summarize_stationarity_from_seed_summaries
from hrdmc.sampling.dmc.run import resolve_parallel_workers
...
from hrdmc.system.settings import DMCRunControls, TrappedCase, controls_to_dict, make_grid
...
def summarize_benchmark_packet_case(
    ...
    *,
    ...
    """Run one DMC packet and derive FW."""
    ...
    if trace_output_dir is not None:
        _write_seed_payload_checkpoints(trace_output_dir, case.case_id, seed_payloads)
...
def summarize_pure_seed_payloads(
    ...
    *,
    ...
    if config is None:
        ...
        observable_configs = dict.fromkeys(observable_names, config)
    aggregates = {
    ...
    observables = {
        name: _summarize_pure_observable(seed_payloads, name, aggregate=aggregates[name])
        for name in observable_names
    }
...
def _summarize_pure_observable(
    seed_payloads: list[dict[str, Any]],
    observable: str,
    *,
...
def _write_seed_payload_checkpoints(
    output_dir: Any,
    case_id: str,
    seed_payloads: list[dict[str, Any]],
) -> None:
