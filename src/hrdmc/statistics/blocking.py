...
def _standard_error_error(stderr: FloatArray, n_blocks: FloatArray) -> FloatArray:
...
def _standard_errors_overlap(
    ...
    *,
    ...
    errors = np.asarray(stderr, dtype=float).reshape(-1)
    error_bars = _standard_error_error(errors, np.asarray(n_blocks, dtype=float).reshape(-1))
