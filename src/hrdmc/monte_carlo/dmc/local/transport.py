...
@dataclass(frozen=True)
class DMCTransportEvent:
    """One DMC-step transport event emitted by the DMC engine.
    ...
    pre-resampling log weights are retained for diagnostics; post-resampling log weights
    define the normalized estimator weights for the emitted positions.

    ``scheduled_move_count`` records how many scheduled transitions have been
    applied so far. It is metadata, not a parent-map boundary. Parent maps have
    single-DMC-step scope and must be consumed event by event.
    """

    step_id: int
    production_step_id: int | None
    scheduled_move_count: int
    ...
    convention: DMCTransportConvention = DMCTransportConvention()
...
class DMCTransportObserver(Protocol):
    def record_transport_event(self, event: DMCTransportEvent) -> None:
        """Consume a transport event without storing full histories by default."""


@dataclass(frozen=True)
class MultiplexedTransportObserver:
    """Fan out DMC transport events to independent estimator consumers."""

    observers: tuple[DMCTransportObserver, ...]
